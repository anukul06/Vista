...
export default function Nav({ onLoginClick }) {
  ...
  const CtaButton = ({ onClick, className }) =>
    isAuthenticated ? (
      <Link to="/dashboard" onClick={onClick} className={className}>
        Dashboard
      </Link>
    ) : (
      <button onClick={onClick ?? onLoginClick} className={className}>
        Login
      </button>
    )
    ...
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
        ...
        <div className="hidden items-center gap-8 md:flex">
          ...
          <CtaButton className="pixel-corners border-2 border-vista-gold bg-vista-gold/10 px-4 py-1.5 font-pixel text-[11px] text-vista-gold transition-colors hover:bg-vista-gold hover:text-vista-night" />
        </div>
